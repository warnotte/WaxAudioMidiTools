...
public class ConfigMIDIINPUT extends JDialog {
	 ...
	 * @param owner
	 */
	public ConfigMIDIINPUT(Frame owner) {
		super(owner,"Configuration MIDI In",true);
		initialize();
	...
	private JComboBox<retourMidiConfig> getJComboBox_OUT() {
		if (jComboBox_OUT == null) {
			...
			jComboBox_OUT.setSelectedIndex(0);
	...
	public static void main(String[] args)
	{
		...
		System.err.println("IN  : "+retour.toString());
	}

	public retourMidiConfig getDonnees() {
		return (retourMidiConfig) jComboBox_OUT.getSelectedObjects()[0];
	}
	
	public static Transmitter createWindow(Transmitter oldtransmitter) throws MidiUnavailableException
	{
		return configTransmitter(oldtransmitter);
	}
	
	public static Transmitter configTransmitter(Transmitter oldtransmitter) throws MidiUnavailableException
	{
		ConfigMIDIINPUT md1 = new ConfigMIDIINPUT(new JFrame());
		md1.pack();
		md1.setVisible(true);
		...
		//Info info = MidiCommon.getMidiDeviceInfo(""+retour[0], false);
		
		MidiDevice device = MidiSystem.getMidiDevice(retour.getInfos());
		device.open();
		
		oldtransmitter = device.getTransmitter();
